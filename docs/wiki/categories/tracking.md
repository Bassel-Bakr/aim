...
Voltaic scores it on scenarios built around small targets. A target that small leaves nowhere to
hide a correction.[^REF-50]

Good execution means the crosshair holds on the target, instead of lagging and snapping forward to
catch up. That's the smoothness quality covered in more depth on
...
traditional subcategory gaps." One scenario strafes on all three axes and the other on two,
and both use rounded direction changes rather than sharp ones.[^REF-50]
