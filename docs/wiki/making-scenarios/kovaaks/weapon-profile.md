...
- **Two tabs add randomness.** Spread and Recoil, unless you use their patterned forms.
- **Type changes the skill.** Hitscan points. Projectile leads.[^REF-86]
- **Cheats are a building tool.** Not something to leave on.[^REF-86]
- **Most tabs are cosmetic.** Graphics and Explosives change nothing about aiming.
...
range, and switch time reduction.[^REF-86]
...
knockback, speed modifiers, stun, tagging and aim punch.
...
weapons to penetrate targets and damage more than one thing.
...
kill, and reload before shot recovery.[^REF-86]
...
and beam tracking. Muzzle flash, wall and body impact particles, decals and decal size.
...
state: spread increase per shot, decrease per second, and a min and max.[^REF-86]
...
reset and time to reset. Manual negation, crouch scale and ADS scale.[^REF-86]
...
max. Crouch Scale and ADS Scale multiply the whole thing in those two states, with one
meaning no change.

**The timing fields have a rule, and the editor states it.** Time To Peak is how long the recoil
takes to reach maximum. Time To Reset is how long after that before the crosshair returns.

The tooltip recommends that peak plus reset comes to less than the Main tab's Time Between
Shots. Break that and the crosshair never gets home between shots, so the climb
accumulates.
...
Per shot recoil: a fixed climb, the kind a Counter-Strike player learns.[^REF-86] The
...
Attaches other weapon profiles to fire alongside this one. Useful for building a weapon out
...
trigger bot and its delay.[^REF-86]
...
Target Preference decides priority when several targets are available. Disable On Kill
decides whether a lock may move to a new target once it has one.

Lock Vertical Offset shifts how far up or down the lock aims when locking vertically.
...
- Reading a score from a weapon with spread as a measure of aim.
